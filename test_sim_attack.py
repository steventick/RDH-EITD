...
def main():
    ...
    filename = os.path.join(data_dir, 'House') #changable
    ...
    for i in range(h * w):
        c_list = []
        for img_index in range(threshold-1): #Simulate malicious adversary, threshold=5
            c_list.append(img_share_decryption_list[img_index, i]) # Add partial decryption results of threshold-1 shares to list
        # Simulate malicious adversary to add a existing image, instead generating a noisy image
    ...
    cv.imwrite(filename + '_adv_rec.png', img_rec)
