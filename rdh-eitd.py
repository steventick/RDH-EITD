...
def main():
    # ------parameter set
    n_bits = 32  # changable, 32, 64, 128, 256, 512
    threshold = 5  # changable, may be large
    n_shares = 7  # changable, may be large
    ve_len = 2  # FirEmb embedding level, [1,2 * n_bits - 8 - 1]
    rr_len2 = 1  # SecEmb embedding level, [1,14], Larger value → drastic rise in computational complexity
    ...
    filename = os.path.join(data_dir, 'Med2') #changable
    ...
    # Simulate resource-constrained terminal above, server simulation below.
    # while True:
    ...
    temp = np.vectorize(int)(img_reconstructed)
    img_rec = temp.astype(np.uint8)
    img_rec = np.reshape(img_rec, (h, w))
    cv.imwrite(filename + '_rec.png', img_rec)
