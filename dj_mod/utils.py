...
def quick_powermod(a, b, m) -> int:
    """ Calculate a^b (mod m) via fast exponentiation.
    :param a: base
    :param b: power
    :param m: modulus
    :return: an integer with the result a^b (mod m).
    """
    result = 1
    a = a % m  # Handle a > m case
    while b > 0:
        # If b is odd: current result *= a
        if b % 2 == 1:
            result = (result * a) % m
        # Right-shift b by one bit and square a.
        b //= 2
        a = (a * a) % m
    return result
