...
from .prime_gen import gen_safe_prime_pair
from .shamir import share_secret
from .utils import int_to_mpz, crm, inv_mod, pow_mod,quick_powermod
...
class PublicKey:
    ...
    @int_to_mpz
    def __init__(self, n: int, s: int, m: int, threshold: int, delta: int, n_inv: int):
    ...
    def encrypt(self, m: int) -> EncryptedNumber:
        ...
        # c = gy.powmod(self.n + 1, m, self.n_s_1) * gy.powmod(r, self.n_s, self.n_s_1) % self.n_s_1
        c = pow(self.n + 1, m, self.n_s_1) * pow(r, self.n_s, self.n_s_1) % self.n_s_1
        # c = quick_powermod(self.n + 1, m, self.n_s_1) * quick_powermod(r, self.n_s, self.n_s_1) % self.n_s_1
...
class PrivateKeyShare:
    ...
    def decrypt(self, c: EncryptedNumber) -> int:
        ...
        """
        # c = self.embed(c)
        #按照论文中份额解密，c_i=c^(2*delta*si) mod n^2
        return pow(c.value, self.two_delta_s_i, self.public_key.n_s_1)
        # return self.quick_powermod(c.value, self.two_delta_s_i, self.public_key.n_s_1)
...
def damgard_jurik_reduce(a: int, s: int, n: int) -> int:
    ...
    """
    def L(b: int) -> int:
        # assert (b - 1) % n == 0 # for simulating attack.
        return (b - 1) // n
...
class PrivateKeyRing:
    def __init__(self, private_key_shares: List[PrivateKeyShare]):
        ...
        self.public_key = public_key #公钥为系统公钥
    ...
    def shareDecrypt(self, c:EncryptedNumber) :
        """share decryption
        :param c: ciphertext
        :return: c_list: all the share decryption results.
        """
        c_list = [pk.decrypt(c) for pk in self.private_key_shares]
        return c_list

    def combiningDecrypt(self, c_list: list):
        """combining decryption
        :param: c_list: the set of share decryption results.
        :return m: plaintext.
        """
        ...
        # Decrypt
        c_prime = mpz(1)
        for c_i, i in zip(c_list, self.i_list):  # c_list: k share decryption values; i_list: corresponding indices
            # c_prime = (c_prime * self.quick_powermod(c_i, (2 * lam(i)), self.public_key.n_s_1)) % self.public_key.n_s_1
...
def keygen(n_bits: int = 64,
    ...
    n = p * q 
    while (len(bin(n))-2 < 2*n_bits):#sjl modified 2024.9.21, to ensure n reaches its maximum possible bit length.
        p, q = gen_safe_prime_pair(n_bits)
        p_prime, q_prime = (p - 1) // 2, (q - 1) // 2
        n = p * q
    m = p_prime * q_prime 
    ...
    public_key = PublicKey(n=n, s=s, m=m, threshold=threshold, delta=delta, n_inv = n_inv)
